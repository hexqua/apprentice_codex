...
5. コード、リソース、依存、datagen に影響する変更では `./gradlew.bat build` を成功させる。ドキュメントのみの変更では省略してよいが、最終報告に理由を残す。
...
9. `main` へ反映する変更は必ずブランチ + PR で流し、直 push しない。PR 作成前にローカルレビューを行う。
...
- ユーザーがローカルレビューを先に行う方針の作業では、明示指示があるまで push、PR 作成、リモート操作を行わない。
