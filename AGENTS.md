...
2. `git cherry-pick -x <sha1> [<sha2> ...]` でコミットを取り込む。
3. コンフリクト時は `1.21.1-main` の既存差分を優先しつつ、必要最小限の手動調整を行う。
4. `./gradlew.bat build` と `./gradlew.bat runData` を実行し、取り込み後の検証成功を確認する。
- 運用ルール:
