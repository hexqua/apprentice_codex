...
- ソースコードに埋め込む GameTest の Assert メッセージ、例外メッセージ、ログなどの開発者向け診断文字列は、既存実装および Minecraft / 外部 MOD の出力との一貫性と検索性を保つため、英語で記述する。テストの意図を説明するコメントは日本語で記述する。
...
- どちらの Skill もデフォルトではレビュー結果の報告だけを行い、明示指示なしに修正、stage、commit、push、PR コメント、merge を行わない。
- コードレビューでは、実行時・ビルド時・データ上の不具合を実装 Finding として優先する。PR 説明、リリースノート、移行ガイド、コミット構成、検証未実行などの運用・文書上の不足だけを実装 Finding にせず、重大な実装不具合を省略する理由にしない。
- ローカル変更レビューでは運用・文書上の不足をデフォルトで報告対象外とする。機能ブランチレビューで必要な場合だけ独立した Note とし、P3 相当を上限として readiness を下げない。
...
- UX のため意図的に client / server 間の差を許容する変更では、許容する挙動と拒否すべき境界の両方を GameTest で明示する。重要な server 側制約が守られた既存仕様として確認できる場合は厳格化を修正方針にせず、テスト不足は検証状況または残余リスクとして示す。判断できない挙動をテスト追加だけで仕様化しない。
- 既存の content ID、設定キー、保存データは原則として維持する。
  - ただし、`mod_version` が `1.0.0` 未満のベータ期間に、1.0.0 正式版へ向けた実装整理として意図的に破壊的変更を行う場合は例外を認める。
  - この例外を適用する変更では、同じ PR または追跡可能なリリース準備で次を明示する。
     - 変更・削除する ID、設定キー、保存データ
     - 既存利用者への影響
     - 置換先または migration 方針。移行経路を提供しない場合は、その旨も明記する
     - リリースノートまたは移行ガイドへの記載
  - 意図した破壊的変更が PR 内で明示されている場合、上記の利用者向け案内が不足しているだけでは実装 Finding または release blocker としない。報告する場合も P3 相当の運用・文書 Note を上限とする。
  - 意図した破壊的変更自体が PR 内で明示されていない場合は原則 P2 相当の実装 Finding とする。案内不足だけを根拠に P1 以上へ引き上げず、実装が意図せず生じさせるデータ破損や機能停止は別途その実害で判定する。
  - `1.0.0` 以降はこの例外を適用せず、実装としての互換経路または migration を必須とする。利用者向け案内の不足は実装上の migration 欠落と区別する。
...
- 関連ドキュメントは原則として実装変更と同じ PR、または追跡可能なリリース準備で更新し、差分の理由を追跡できる状態にする。案内の更新漏れだけではコードレビューの実装 Finding または release blocker としない。
