...
public final class SpellgunAmmoHudRenderEvent {
    ...
    public static void onRenderGuiOverlay(RenderGuiOverlayEvent.Post event) {
        if (event.getOverlay() != VanillaGuiOverlay.CROSSHAIR.type() || !ApprenticeCodexClientConfig.enableSpellgunAmmoHud()) {
            return;
        }
        ...
        var centerX = event.getWindow().getGuiScaledWidth() / 2;
        var centerY = event.getWindow().getGuiScaledHeight() / 2;

        guiGraphics.pose().pushPose();
        guiGraphics.pose().translate(0.0F, 0.0F, 200.0F);

        var mainHandAmmo = resolveAmmoDisplay(player.getMainHandItem(), player, event.getPartialTick());
        if (mainHandAmmo != null) {
            renderAmmoPanel(guiGraphics, minecraft.font, mainHandAmmo, centerX + CROSSHAIR_GAP, centerY - PANEL_HEIGHT / 2);
        }

        var offHandAmmo = resolveAmmoDisplay(player.getOffhandItem(), player, event.getPartialTick());
        if (offHandAmmo != null) {
