...
public final class StorageStabilizerSelectionClientController {
    private static final ResourceLocation TEXTURE =
            ResourceLocation.fromNamespaceAndPath(ApprenticeCodex.MODID, "textures/gui/storage_stabilizer.png");
    ...
    private StorageStabilizerSelectionClientController() {
    }
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        if (activeState != null) {
            if (minecraft.screen != null || ClientMagicData.isCasting() || !isValidHeldStabilizer(player, activeState.hand())) {
                clearState();
            } else {
    ...
    private static void openSelection(net.minecraft.world.entity.player.Player player, InteractionHand hand) {
        ...
        var views = StorageStabilizer.getSelectionViews(stack);
        if (views.isEmpty()) {
            return;
        }
    ...
    private static void refreshActiveState(net.minecraft.world.entity.player.Player player) {
        ...
        var selectedSpellIndex = activeState.selectedView().spellIndex();
        var refreshedViews = StorageStabilizer.getSelectionViews(player.getItemInHand(activeState.hand()));
        if (refreshedViews.isEmpty()) {
            clearState();
            return;
        }

        var selectedViewIndex = indexOfSpell(refreshedViews, selectedSpellIndex);
    ...
    private static boolean isValidHeldStabilizer(net.minecraft.world.entity.player.Player player, InteractionHand hand) {
        return player.getItemInHand(hand).getItem() instanceof StorageStabilizer;
    }

    private static int findInitialViewIndex(List<StorageStabilizer.SpellSelectionView> views) {
    ...
    private static int indexOfSpell(List<StorageStabilizer.SpellSelectionView> views, int spellIndex) {
        for (var index = 0; index < views.size(); ++index) {
            if (views.get(index).spellIndex() == spellIndex && views.get(index).hasSpell()) {
                return index;
            }
        }
        return -1;
    ...
    private static void renderSlot(GuiGraphics guiGraphics, int rowX, int rowY, int column, int slotCount,
                                   StorageStabilizer.SpellSelectionView view) {
    ...
    private record ActiveSelectionState(
            ...
            List<StorageStabilizer.SpellSelectionView> views,
        ...
        private StorageStabilizer.SpellSelectionView selectedView() {
