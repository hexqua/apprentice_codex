...
final class AutoTurretPlacementHelper {
    ...
    public static Optional<PlacementResult> resolveClientPreview(Level level, LivingEntity entity, double range) {
        ...
        return raycastTargetBlock(level, entity, range)
                .flatMap(hit -> resolve(level, hit.getBlockPos(), hit.getDirection(), hit.getLocation()));
    }
    ...
    public static Optional<PlacementResult> resolve(Level level, BlockTargetData targetData) {
        ...
        return resolve(level, targetData.getHitBlockPos(), targetData.getHitFace(), targetData.getHitLocation());
    }

    private static Optional<PlacementResult> resolve(Level level, BlockPos hitPos, Direction hitFace, Vec3 hitLocation) {
