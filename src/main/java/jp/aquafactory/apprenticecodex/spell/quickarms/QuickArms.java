...
public class QuickArms extends AbstractSummonWeaponRecastSpell<QuickArmsHandgunEntity> implements IMagiAgentSuitAffectedSpell {
    ...
    @Override
    protected boolean onPreRecastWithWeapon(Level level, int spellLevel, LivingEntity entity, MagicData playerMagicData,
    ...
    @Override
    public void onCastWithWeapon(Level level, int spellLevel, LivingEntity entity, MagicData playerMagicData,
                                 @NotNull QuickArmsHandgunEntity weapon){
        // FocusStaffbow のリキャスト詠唱は castSpell 中だけ SPELL_POWER を増やすため、射撃直前に計算する。
        weapon.fire(level, getDamage(spellLevel, entity));
    }
    ...
    public QuickArmsHandgunEntity onCastNoWeapon(Level level, int spellLevel, LivingEntity entity, MagicData playerMagicData){
        ...
        summonWeapon.setFireStandby(getFirstDelay(), getDamage(spellLevel, entity));
