...
public class GrindRunnerWheelEntity extends SummonWeaponEntity implements GeoEntity {
    ...
    private Optional<ItemStackProcessingResult> tryBuildCreateProcessingResult(ServerLevel level, ItemStack inputStack, int maxProcessCount) {
        ...
        for (var recipeTypeId : List.of(CREATE_CRUSHING_RECIPE_TYPE_ID, CREATE_MILLING_RECIPE_TYPE_ID)) {
            ...
            var createOutputs = rollCreateProcessingOutputs(level, createRecipe.get(), processCount);
    ...
    private Optional<Recipe<?>> findCreateProcessingRecipe(ServerLevel level, ItemStack inputStack, ResourceLocation recipeTypeId) {
        if (!ModList.get().isLoaded(CREATE_MOD_ID)) {
            return Optional.empty();
        }

        RecipeType<?> createRecipeType = ForgeRegistries.RECIPE_TYPES.getValue(recipeTypeId);
        if (createRecipeType == null) {
            return Optional.empty();
        }

        return level.getRecipeManager().getRecipes().stream()
                .filter(recipe -> recipe.getType() == createRecipeType)
                .filter(recipe -> matchesCreateProcessingInput(recipe, inputStack))
                .findFirst();
    }
    ...
    private void performDamage(LivingEntity owner, float damageAmount, CombatTools.KnockbackTypes knockbackType) {
        ...
        for (var target : resolveDamageTargets(owner, includeOwner)) {
            CombatTools.applyDamage(target, damageAmount, source, SpellRegistry.GRIND_RUNNER.get().getSchoolType(),
                    knockbackType, policy);
        }
    }
    ...
    private LinkedHashSet<LivingEntity> resolveDamageTargets(LivingEntity owner, boolean includeOwner) {
        ...
        var hits = RaycastTools.sampleBeamHits(
                ...
                DAMAGE_SIDE_RADIUS,
                DAMAGE_SAMPLE_STEP,
                e -> CombatTools.isValidCombatTarget(e, owner, includeOwner
                        ? CombatTools.CombatTargetPolicy.ALLOW_SELF_PROTECT_ALLIES
                        : CombatTools.CombatTargetPolicy.PROTECT_SELF_AND_ALLIES)
        );
