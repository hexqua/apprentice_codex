...
public final class CurioLootDataGenerator implements DataProvider {
    ...
    public CurioLootDataGenerator(PackOutput output) {
        lootTablePathProvider = output.createPathProvider(PackOutput.Target.DATA_PACK, "loot_tables");
    ...
    private static JsonObject createChanceWrappedTable(ResourceLocation nestedLootTableId) {
        ...
        entry.addProperty("name", nestedLootTableId.toString());
        entries.add(entry);
        pool.add("entries", entries);
        pools.add(pool);
        root.add("pools", pools);
    ...
    private static JsonObject createLootTableCondition(List<ResourceLocation> lootTableIds) {
        if (lootTableIds.size() == 1) {
            ...
            condition.addProperty("condition", "forge:loot_table_id");
            condition.addProperty("loot_table_id", lootTableIds.get(0).toString());
            return condition;
        }
        ...
        for (var lootTableId : lootTableIds) {
            ...
            term.addProperty("condition", "forge:loot_table_id");
