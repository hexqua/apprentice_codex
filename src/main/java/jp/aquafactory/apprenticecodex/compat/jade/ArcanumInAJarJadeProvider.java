...
public enum ArcanumInAJarJadeProvider implements IBlockComponentProvider, IServerDataProvider<BlockAccessor> {
    ...
    public void appendServerData(CompoundTag data, BlockAccessor accessor) {
        ...
        data.put(MATERIAL_ITEM_TAG, new ItemStack(settings.materialItem()).save(new CompoundTag()));
        //noinspection DataFlowIssue
        data.put(PRODUCT_ITEM_TAG, new ItemStack(settings.productItem()).save(new CompoundTag()));
    ...
    public void appendTooltip(ITooltip tooltip, BlockAccessor accessor, IPluginConfig config) {
        ...
        if (data.getBoolean(VALID_SETTINGS_TAG)) {
            appendConfiguredItemLine(tooltip, data, MATERIAL_ITEM_TAG, MATERIAL_COUNT_TAG);
            appendConfiguredItemLine(tooltip, data, PRODUCT_ITEM_TAG, PRODUCT_COUNT_TAG);
        } else {
    ...
    private static void appendConfiguredItemLine(
        ...
        var stack = ItemStack.of(data.getCompound(itemTag));
        if (stack.isEmpty()) {
            return;
        }
