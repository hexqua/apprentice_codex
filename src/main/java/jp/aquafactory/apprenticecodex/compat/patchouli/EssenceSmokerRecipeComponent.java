...
public final class EssenceSmokerRecipeComponent implements ICustomComponent {
    ...
    public void onVariablesAvailable(UnaryOperator<IVariable> lookup, HolderLookup.Provider registries) {
        ...
        collectConfiguredRecipeId(parsedIds, lookup, registries, recipe3);

        if (parsedIds.isEmpty()) {
            for (var configuredRecipeId : configuredRecipeIds) {
                collectConfiguredRecipeId(parsedIds, lookup, registries, configuredRecipeId);
        ...
        if (parsedIds.isEmpty()) {
            ...
            collectLookupRecipeId(parsedIds, lookup, registries, "recipe3");
        }

        if (parsedIds.isEmpty()) {
            var variables = lookup.apply(IVariable.wrap("recipe_ids", registries)).asListOrSingleton(registries);
    ...
    private void refreshRecipes() {
        ...
        for (var recipeId : recipeIds) {
            ...
            resolved.add(DisplayRecipe.found(recipeId, match.value()));
        }
    ...
    private static void collectConfiguredRecipeId(
        ...
        var resolvedValue = lookup.apply(IVariable.wrap(rawValue, registries)).asString("").trim();
    ...
    private static void collectLookupRecipeId(
        ...
        var resolvedValue = lookup.apply(IVariable.wrap(key, registries)).asString("").trim();
