...
public class CompoundPhialProjectileRenderer extends EntityRenderer<CompoundPhialProjectileEntity> {
    ...
    private final ItemStack renderItem = new ItemStack(Items.SPLASH_POTION);
    private boolean isAppliedColor = false;
    ...
    public void render(@NotNull CompoundPhialProjectileEntity entity, float entityYaw, float partialTicks,
        ...
        if (!isAppliedColor) {
            // todo:Entity側にアイテム情報を持つ.
            renderItem.getOrCreateTag().putInt("CustomPotionColor", 0x33D6FF);
            isAppliedColor = true;
        }
