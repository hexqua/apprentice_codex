...
public final class SacredArrowGameTests {
    ...
    public static void selectionIsNearestAndFrozenAtStart(GameTestHelper h) {
        try (var s = new Scene(h)) {
            ...
            near.addEffect(new MobEffectInstance(MobEffectRegistry.GUIDING_BOLT.get(), 300));
            far.addEffect(new MobEffectInstance(EffectRegistry.SACRED_SIGN.get(), 300));
            s.owner.addEffect(new MobEffectInstance(EffectRegistry.SACRED_SIGN.get(), 300));
            var data = s.preCast();
            ...
            near.removeEffect(MobEffectRegistry.GUIDING_BOLT.get());
        ...
        h.runAfterDelay(2, () -> {
            try {
                try (var s = new Scene(h)) {
                    ...
                    var selected = ((SacredArrowCastData) s.preCast().getAdditionalCastData()).targetId();
                    h.assertTrue(target.getUUID().equals(selected), "Inside the 48 block AABB must lock: selected=" + selected
                            ...
                            + ", owner=" + s.owner.position() + ", target=" + target.position()
                            + ", look=" + s.owner.getLookAngle());
    ...
    public static void normalDamageMarksOnlySuccessfulLivingHits(GameTestHelper h) {
        try (var s = new Scene(h)) {
            ...
            step(arrow);
            h.assertTrue(arrow.isRemoved() && Math.abs(target.getHealth() - 12) < 0.01, "Normal hit must deal full damage and consume the arrow arrow=" + arrow.position() + ", target=" + target.position() + ", health=" + target.getHealth() + ", removed=" + arrow.isRemoved());
            h.assertTrue(target.getEffect(EffectRegistry.SACRED_SIGN.get()).getDuration() == 300, "Normal hit must apply a 15 second mark");
            target.removeEffect(EffectRegistry.SACRED_SIGN.get());
            target.setInvulnerable(true);
            arrow = s.arrow(null, new Vec3(1, 0, 0)); step(arrow);
            h.assertTrue(arrow.isRemoved() && !target.hasEffect(EffectRegistry.SACRED_SIGN.get()), "Failed damage must not mark or pierce");
        }
        h.succeed();
    }
    ...
    public static void piercingStopsAtPrimaryAndNeverRefreshesMarks(GameTestHelper h) {
        try (var s = new Scene(h)) {
            ...
            target.addEffect(new MobEffectInstance(EffectRegistry.SACRED_SIGN.get(), 123));
            ...
            h.assertTrue(target.getEffect(EffectRegistry.SACRED_SIGN.get()).getDuration() == 123 && !bystander.hasEffect(EffectRegistry.SACRED_SIGN.get()), "Homing hits must not apply or refresh marks");
        }
        h.succeed();
    }
    ...
    public static void captureConeKeepsDistancePriorityAndNormalFallback(GameTestHelper h) {
        try (var s = new Scene(h)) {
            ...
            near.addEffect(new MobEffectInstance(MobEffectRegistry.GUIDING_BOLT.get(), 300));
            center.addEffect(new MobEffectInstance(EffectRegistry.SACRED_SIGN.get(), 300));
            behind.addEffect(new MobEffectInstance(EffectRegistry.SACRED_SIGN.get(), 300));
    ...
    private static final class Scene implements AutoCloseable {
        ...
        Scene(GameTestHelper h) {
            helper = h; origin = h.absoluteVec(new Vec3(2, 250, 2));
