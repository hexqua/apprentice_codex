...
final class ScytheMalumExclusiveGameTestScenarios {
    ...
    public static void geasTriggersOnlyForReboundAndRetainsDelayedSource(GameTestHelper h) {
        ...
        var data = h.getLevel().getData(LodestoneAttachmentTypes.WORLD_EVENT_DATA);
        var created = new ArrayList<WorldEventInstance>();
        var scythe = new ScytheThrowEntity(EntityRegistry.SCYTHE_THROW.get(), h.getLevel());
        try {
            h.assertTrue(GeasEffectHandler.addGeasEffect(p,
                    MalumGeasEffectTypes.PACT_OF_THE_REAPER.get()), "Test Reaper pact must equip");
            long seed = 0;
            // 近接30%では失敗し投擲60%では成功する値を固定し、確率試行をテストに持ち込まない。
            while (true) {
                float sample = RandomSource.create(seed).nextFloat();
            ...
            Consumer<LivingDamageEvent.Pre> listener = event -> {
                if (event.getEntity() == target && event.getSource().is(MalumDamageTypes.SCYTHE_COMBO)) {
    ...
    public static void profaneGluttonConsumesStacksOnDelayedCut(GameTestHelper h) {
        ...
        try {
            h.assertTrue(GeasEffectHandler.addGeasEffect(p,
                    MalumGeasEffectTypes.PACT_OF_THE_PROFANE_GLUTTON.get()), "Test Glutton pact must equip");
            p.addEffect(new MobEffectInstance(MalumMobEffects.DESPERATE_NEED, 200, 6));
            for (var mode : new ScytheThrowEntity.Mode[]{ScytheThrowEntity.Mode.NORMAL, ScytheThrowEntity.Mode.REBOUND}) {
                scythe.prepare(p, p.getMainHandItem(), 0, 16, mode);
            ...
            target.hurt(CombatTools.getDamageSource(h.getLevel(), scythe, p,
                    MalumDamageTypes.SCYTHE_MAELSTROM), 2);
            ...
            h.assertTrue(target.hasEffect(MobEffects.POISON), "Glutton cut must apply native poison");
            h.assertTrue(p.getEffect(MalumMobEffects.DESPERATE_NEED).getAmplifier() == 3,
    ...
    public static void maelstromPulseAndNativeStormRespectAllies(GameTestHelper h) {
        ...
        for (var target : new Cow[]{enemy, ally}) {
        ...
        h.runAfterDelay(delay, () -> {
            ...
            try {
                ...
                CurioHowlingMaelstromRing.handleMaelstrom(h.getLevel(), p, storm);
