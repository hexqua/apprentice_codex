...
final class ChargedStaffRiptideGameTestScenarios extends ApprenticeCodexGameTestScenarios {
    ...
    static void upkeepAndExhaustion(GameTestHelper helper) {
        ...
        var stack = equip(helper, player, 3);
        ...
        for (int tick = 2; tick <= 62; tick++) {
            ...
            helper.runAtTickTime(tick, () -> {
                ...
                player.aiStep();
                if (current <= 60) {
                    helper.assertTrue(player.isAutoSpinAttack(), "Sustained Riptide ended before mana exhaustion");
    ...
    static void independentCostsAndFreeUpkeep(GameTestHelper helper) {
        ...
        var stack = equip(helper, player, 1);
        var data = MagicData.getPlayerMagicData(player);
        data.setMana(10);
    ...
    static void equipmentAndCreativeBoundaries(GameTestHelper helper) {
        helper.succeedIf(() -> {
            ...
            var stack = equip(helper, player, 2);
            ...
            player.releaseUsingItem();
            player.startAutoSpinAttack(20, 8, stack);
            stack.getItem().use(player.level(), player, InteractionHand.MAIN_HAND);
            helper.assertTrue(!ChargedTwinBladeStaffRiptide.isMaintenanceInput(player),
    ...
    static void cruiseSpeedAndSteering(GameTestHelper helper) {
        ...
        for (int level : new int[]{1, 2, 3, 5}) {
            for (int pitch : new int[]{0, -45, -90}) {
                ...
                var stack = equip(helper, player, level);
                player.getAbilities().instabuild = true;
                ...
                for (int tick = 1; tick <= 80; tick++) {
                    ...
                    helper.runAtTickTime(tick, () -> {
                        ...
                        player.setDeltaMovement(velocity.subtract(0, player.getGravity(), 0).multiply(0.91, 0.98, 0.91));
                    });
                }
            }
        }
    ...
    static void tooltipStylesAndCosts(GameTestHelper helper) {
        helper.succeedIf(() -> {
            ...
            stack.enchant(helper.getLevel().registryAccess().lookupOrThrow(Registries.ENCHANTMENT)
                    .getOrThrow(Enchantments.CHANNELING), 1);
            try (var ignored = config(73, Integer.MAX_VALUE)) {
                var lines = new ArrayList<Component>();
                stack.getItem().appendHoverText(stack, Item.TooltipContext.of(helper.getLevel()), lines, TooltipFlag.NORMAL);
    ...
    private static ItemStack equip(GameTestHelper helper, Player player, int level) {
        // MagicData#setManaは最大マナでclampするため、複数回の支払いを検証できる容量を明示する。
        Objects.requireNonNull(player.getAttribute(AttributeRegistry.MAX_MANA)).setBaseValue(500);
        var stack = new ItemStack(ItemRegistry.CHARGED_TWIN_BLADE_STAFF.get());
        stack.enchant(helper.getLevel().registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(Enchantments.RIPTIDE), level);
    ...
    private static void chargeAndRelease(Player player, ItemStack stack) {
        stack.getItem().use(player.level(), player, InteractionHand.MAIN_HAND);
        ((LivingEntityAccessor) player).apprenticecodex$setUseItemRemaining(stack.getUseDuration(player) - ChargedTwinBladeStaff.THROW_THRESHOLD_TICKS);
        player.releaseUsingItem();
    }
}
